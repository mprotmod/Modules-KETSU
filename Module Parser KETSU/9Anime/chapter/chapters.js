...
if (url.href.includes('google')) {
    ...
    function transform(r) { const e = r.substr(0, 16); let t = r.substr(16); if ((t = t.replace(/[ \\t\\n\\f\\r]/g, '')).length % 4 == 0 && ((t = t.replace(/==?$/, '')).length % 4 == 1 || /[^+/0-9A-Za-z]/.test(t))) return null; let n = '', o = 0; for (let r = 0; r < t.length; r++) { o <<= 6; const e = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/'.indexOf(t[r]); if (((o |= e < 0 ? void 0 : e), (r + 1) % 4 == 0 && ((n += String.fromCharCode((16711680 & o) >> 16)), (n += String.fromCharCode((65280 & o) >> 8)), (n += String.fromCharCode(255 & o)), (o = remaining = 0)), r == t.length - 1)) switch (r % 4) { case 1: (o >>= 4), (n += String.fromCharCode(o)); break; case 2: (o >>= 2), (n += String.fromCharCode((65280 & o) >> 8)), (n += String.fromCharCode(255 & o)); } } try { n = decodeURIComponent(n); } catch (r) {} var a = [...Array(256).keys()], i = 0; for (let r = 0; r < 256; r += 1) a[(i = (i + a[r] + e.charCodeAt(r % e.length)) % 256)] = [a[r], (a[r] = a[i])][0]; for (var d = '', h = (i = index = 0); h < n.length; h += 1) (a[(i = (i + a[(index = (index + h) % 256)]) % 256)] = [a[index], (a[index] = a[i])][0]), (d += String.fromCharCode(n.charCodeAt(h) ^ a[(a[index] + a[i]) % 256])); return d;}
